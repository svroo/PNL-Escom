...
def probability_word(text, word, ocurs = True):
    ...
    if ocurs:
        probability = len(wordocurs) / len(text)
    else:
        probability = 1 - (len(wordocurs) / len(text))
    return probability
...
def get_entropies(text, word1, vocabulary):
    ...
    with open('./entropies/' + word1 + '.txt', 'w', encoding = 'utf-8') as f:
